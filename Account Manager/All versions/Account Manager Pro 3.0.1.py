...
root.title("Account Manager Pro")
...
def UserSetting():
    ...
    def chPWState():
        ...
        if PWState == False:
            ...
            if Check_PWFormError() != True:
                ...
                PWLabel.set(u'変更')
                PWState = False
    def updateEncryptKey():
        if messagebox.askokcancel(u'確認', u'続行しますか？'):
            global AccountKey, AccountIV, UserKey, UserIV
            NewUKey = ''.join(random.choice(KeySource) for i in range(32))
            NewUIV = ''.join(random.choice(KeySource) for i in range(16))
            NewUobj = AES.new(NewUKey, AES.MODE_CBC, NewUIV)
            if UserPW.get() != '':
                NewUserPW = NewUobj.encrypt(ManagePlain(UserPW.get()))
            else:
                NewUserPW = ''
            NewAKey = ''.join(random.choice(KeySource) for i in range(32))
            NewAIV = ''.join(random.choice(KeySource) for i in range(16))
            d = (NewUserPW, NewAKey, NewAIV, NewUKey, NewUIV)
            try:
                UserDB.execute('update user set PW=?, AccountKey=?, AccountIV=?, UserKey=?, UserIV=?', d)
                accountDB.execute('select count (Service) from account')
                for count in accountDB.fetchone():
                    if count > 0:
                        for i in range(count):
                            accountDB.execute('select id, PW from account where No =?', (i + 1,))
                            for id, pw in accountDB.fetchall():
                                Aobj = AES.new(AccountKey, AES.MODE_CBC, AccountIV)
                                NewAobj = AES.new(NewAKey, AES.MODE_CBC, NewAIV)
                                Aid = Aobj.decrypt(id).decode('utf-8')
                                APW = Aobj.decrypt(pw).decode('utf-8')
                                NewAid = NewAobj.encrypt(Aid)
                                NewAPW = NewAobj.encrypt(APW)
                                Aobj = None
                                NewAobj = None
                                accountDB.execute('update account set id=?, PW=? where No=?', (NewAid, NewAPW, i + 1))
            except:
                messagebox.showerror(u'エラー', u'エラーが発生しました。', parent=SecurityWindow)
            else:
                AccountKey = NewAKey
                AccountIV = NewAIV
                UserKey = NewUKey
                UserIV = NewUIV
                messagebox.showerror(u'完了', u'更新が完了しました。', parent=SecurityWindow)
    ...
    if SecurityWindow is None or not SecurityWindow.winfo_exists():
        ...
        SecurityWindow.minsize(400, 200)
        Base = Frame(SecurityWindow)
        Form = Frame(Base)
        Labels = Frame(Form)
        Insert = Frame(Form)
        Edits = Frame(Form)
        SecurityF = Frame(Base)
        ...
        UserPWForm.pack()
        Label(SecurityF, text=u'セキュリティ', font=('Meiryo', 12)).pack()
        Label(SecurityF, text=u'途中で問題が起きると、データを復元出来なくなる可能性があるため、\n更新の前にバックアップの作成を推奨。').pack()
        Button(SecurityF, text=u'暗号情報の更新', command = updateEncryptKey).pack()
...
def choose_account(event):
    ...
    for Service, id, pw in accountDB.fetchall():
        try:
            ...
            messagebox.showerror(u'エラー', u'データの読み込みに失敗しました -001')
        else:
            ServiceName.set(Service)
            editB.configure(state='normal')
            hide.configure(state='normal')
...
def CreateBackup():
    global account, user_data
    try:
        os.mkdir('%s/Account Manager/backup' % DataPath)
    except:
        pass
    if os.path.exists('%s/Account Manager/backup/data1.db' % DataPath):
        os.remove('%s/Account Manager/backup/data1.db' % DataPath)
    if os.path.exists('%s/Account Manager/backup/data2.db' % DataPath):
        os.remove('%s/Account Manager/backup/data2.db' % DataPath)
    try:
        account.close()
        user_data.close()
        shutil.copy2('%s/Account Manager/data/data1.db' % DataPath, '%s/Account Manager/backup/data1.db' % DataPath)
        shutil.copy2('%s/Account Manager/data/data2.db' % DataPath, '%s/Account Manager/backup/data2.db' % DataPath)
    except:
        messagebox.showerror(u'エラー', u'バックアップ作成に失敗しました。')
    else:
        messagebox.showinfo(u'完了', u'バックアップは正常に作成されました。')
def RestoreFromBackup():
    ...
    try:
        account.close()
        user_data.close()
        os.remove('%s/Account Manager/data/data1.db' % DataPath)
        os.remove('%s/Account Manager/data/data2.db' % DataPath)
        ...
        messagebox.showinfo(u'完了', u'復元に成功しました。')
        ConnectDB()
        update_list()
def OpenWindow():
...
m3.add_command(label=u'License', command=UserManual)
m3.add_command(label=u'Readme', command=UserManual)
...
Label(MainUI, text="Account Manager Pro", font=('Meiryo', 18)).pack()
...
EditBox.pack()
Label(Labels, text=u'アカウント名：', font=('Meiryo', 13)).pack(anchor='e')
...
def load_UI():
    ...
    m1.add_separator()
    m1.add_command(label=u'バックアップの作成', command = CreateBackup)
    m1.add_command(label=u'バックアップから復元', command = RestoreFromBackup)
