...
def user_manage():
    ...
    if w5 is None or not w5.winfo_exists():
        ...
        Label(add, text = u'ユーザー追加').pack()
        Label(lists, text = u'ユーザー一覧').pack()
        Label(name, text = u'ユーザー名及びID:').pack(side = LEFT)
        Entry(name, textvariable = add_name).pack(side = RIGHT)
        Label(pw, text = u'パスワード(未設定可):').pack(side = LEFT)
        Entry(pw, textvariable = add_pw).pack(side = RIGHT)
        Label(login, text = u'ログイン:').pack(side = LEFT)
        Radiobutton(login, text = u'しない', variable = add_login, value = 2).pack(side = RIGHT)
        Radiobutton(login, text = u'する', variable = add_login, value = 1).pack(side = RIGHT)
        Label(authority, text = u'管理者権限:').pack(side = LEFT)
        Radiobutton(authority, text = u'持たせない', variable = add_authority, value = 2).pack(side = RIGHT)
        Radiobutton(authority, text = u'持たせる', variable = add_authority, value = 1).pack(side = RIGHT)
        Button(b, text = u'追加').pack()
        add.pack(side = LEFT)
...
m2.add_command(label = u'ユーザー管理', command = user_manage, state = 'disabled')
...
#--------------------------------ここまでがUIの定義------------------------------
#2回目の処理:以下でログイン設定が有効ならログイン、しなければタブ及びタブ内の定義を設置
user_data.execute("select login from user")
