...
def security():
    if st1.get() and st2.get():
        ...
        for ii in fo2:
            ii == ii
        if st1.get() == i and st2.get() == ii:
            menu()
            w1.destroy()
        else:
            Message(w1, text = '管理社名又はパスワードが違います').pack()
    else:
...
f3 = Frame(root)
Label(f3, text = 'Personal TradeBox', font = ('New Times Roman', 24)).pack()
Label(f3, text = '利用方法:').pack()
Label(f3, text = '1.購入する商品の値段分のお金を投入').pack()
Label(f3, text = '2.商品のボタンを押す').pack()
Label(f3, text = '3.ロッカーが開くので商品を取り出す').pack()
f6 = Frame(root)
Label(f6, text = '合計金額:').pack(side = LEFT)
...
f9 = Frame(root)
Label(f9, text = '所持額:').pack(side = LEFT)
Label(f9, text = '0円').pack(side = RIGHT)
f10 = Frame(root)
...
Label(f10, text = '＊ボタンを押してお金を投入してください', font = ('New Times Roman', 18), bg = 'red').pack()
...
f11 = Frame(root)
Button(f11, text = '取引終了', command = sys.exit).pack()
Button(f11, text = '管理(管理者のみ)', command = owner).pack()
